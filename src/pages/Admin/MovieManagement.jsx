...
import { managerAPI } from "../../services/api"; // Sử dụng managerAPI thay vì các hàm riêng lẻ
...
const MovieManagement = () => {
  ...
  const fetchMovies = async () => {
    try {
      const response = await managerAPI.getMovies(); // Sử dụng managerAPI
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedMovie) {
        ...
        await managerAPI.updateMovie(selectedMovie.ma_phim, data); // Sử dụng managerAPI
        toast.success("Cập nhật phim thành công");
      } else {
        await managerAPI.createMovie(data); // Sử dụng managerAPI
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa phim này?")) {
      try {
        await managerAPI.deleteMovie(id); // Sử dụng managerAPI
