...
const ManageTickets = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      let response;
      if (nguoiDung?.ma_vai_tro === 2) {
        // 2 là mã vai trò quản lý
        response = await managerAPI.getManagerTickets();
      } else {
  ...
  const filterTickets = () => {
    const filtered = tickets.filter((ticket) => {
      ...
      return (
        ...
        (ticket.khach_hang?.ten || "").toLowerCase().includes(searchString) ||
  ...
  const handleStatusChange = async (ticketId, newStatus) => {
    try {
      await managerAPI.updateTicketStatus(
        ticketId,
